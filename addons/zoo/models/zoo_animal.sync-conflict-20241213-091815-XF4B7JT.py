...
class ZooAnimal(models.Model):
    ...
    doctor_id = fields.Many2one('res.partner',comodel_name='zoo.animal', string='Docters')
    ...
    def _check_parent_ids(self):
        for record in self:
            ...
            if record.father_id and record.father_id == record.id:
                raise ValidationError(_("Animal cannot be its own father!"))
            if record.mother_id and record.mother_id == record.id:
    ...
    @api.constrains('female_children_ids', 'male_children_ids')
